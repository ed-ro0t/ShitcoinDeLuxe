...
ShitcoinDeLuxe Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- BitcoinMiner : Generates shitcoins (if wallet is enabled).
...
However, in open source software such as ShitcoinDeLuxe Core, where everyone uses
...
pay attention to for reviewers of ShitcoinDeLuxe Core code.

General ShitcoinDeLuxe Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: ShitcoinDeLuxe Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of ShitcoinDeLuxe Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against ShitcoinDeLuxe Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:ShitcoinDeLuxe-project/ShitcoinDeLuxe.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ShitcoinDeLuxe-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ShitcoinDeLuxe-cli` and the GUI debug console use this table to determine how to
