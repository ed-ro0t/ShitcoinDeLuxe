...
*Setup instructions for a Gitian build of ShitcoinDeLuxe Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ShitcoinDeLuxe
...
- [Building ShitcoinDeLuxe Core](#building-ShitcoinDeLuxe-core)
...
Clone the git repositories for ShitcoinDeLuxe and Gitian.
...
in the ShitcoinDeLuxe repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building ShitcoinDeLuxe Core
----------------

To build ShitcoinDeLuxe Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ShitcoinDeLuxe repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ShitcoinDeLuxe/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ShitcoinDeLuxe-project/ShitcoinDeLuxe
...
For example:
```bash
URL=https://github.com/thrasher-/ShitcoinDeLuxe.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit ShitcoinDeLuxe=${COMMIT} --url ShitcoinDeLuxe=${URL} ../ShitcoinDeLuxe/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ShitcoinDeLuxe=${COMMIT} --url ShitcoinDeLuxe=${URL} ../ShitcoinDeLuxe/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ShitcoinDeLuxe=${COMMIT} --url ShitcoinDeLuxe=${URL} ../ShitcoinDeLuxe/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the ShitcoinDeLuxe git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ShitcoinDeLuxe/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/ShitcoinDeLuxe
SIGPATH=/some/root/path/ShitcoinDeLuxe-detached-sigs

./bin/gbuild --url ShitcoinDeLuxe=${BTCPATH},signature=${SIGPATH} ../ShitcoinDeLuxe/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ShitcoinDeLuxe-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ShitcoinDeLuxe-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ShitcoinDeLuxe-osx-build.assert
```
...
[ShitcoinDeLuxe-project/gitian.sigs.ltc](https://github.com/ShitcoinDeLuxe-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
