ShitcoinDeLuxe Core version 0.13.2 is now available from:

  <https://download.ShitcoinDeLuxe.org/ShitcoinDeLuxe-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a ShitcoinDeLuxe
...
ECDSA signatures inside ShitcoinDeLuxe transactions now use validation using
...
Previous versions of ShitcoinDeLuxe Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of ShitcoinDeLuxe Core was to raise the default minimum
relay fee.

ShitcoinDeLuxe Core 0.13.2 will have a strict maximum size on the mempool. The
...
ShitcoinDeLuxe Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running ShitcoinDeLuxed without having to do any manual configuration.
...
ShitcoinDeLuxe Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. ShitcoinDeLuxe Core will relay transactions with
...
entirely in ShitcoinDeLuxe Core 0.13, and it is at this time undecided whether the
...
ShitcoinDeLuxe Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
ShitcoinDeLuxe Core automatically creates a hidden service to listen on, without
manual configuration. ShitcoinDeLuxe Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if ShitcoinDeLuxe Core is listening, and
...
SHITCOINDELUXED can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals ShitcoinDeLuxe
Core to use floating fees. By default, ShitcoinDeLuxe Core will use floating
fees.
...
At all times, ShitcoinDeLuxe Core will cap fees at `-maxtxfee=<x>` (default:
0.10) SHT.
Furthermore, ShitcoinDeLuxe Core will never create transactions paying less than
...
`ShitcoinDeLuxe.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /litecoinrpc>
        ...
        # optional bypass ShitcoinDeLuxed rpc basic auth
...
Restarting ShitcoinDeLuxed will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in ShitcoinDeLuxe.conf
...
ShitcoinDeLuxe-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/ShitcoinDeLuxe-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The ShitcoinDeLuxe Core code base has
...
- `ShitcoinDeLuxe-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `ShitcoinDeLuxe-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the ShitcoinDeLuxe scripting system that in combination with
...
ShitcoinDeLuxe transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of ShitcoinDeLuxe Core.
...
The two phases are distinct in the ShitcoinDeLuxe-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming ShitcoinDeLuxe wallet
  software more difficult and which seriously complicates the design of smart
  contracts for ShitcoinDeLuxe.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each ShitcoinDeLuxe block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of shitcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of shitcoins being spent is exactly
...
- **Increased security for multisig:** ShitcoinDeLuxe addresses (both P2PKH addresses
  ...
  believed to be provided by ShitcoinDeLuxe's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of ShitcoinDeLuxe's current ledger (called the UTXO set),
  ...
  note that ShitcoinDeLuxe Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  ShitcoinDeLuxe users to individually opt-in to almost any change in the ShitcoinDeLuxe
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and ShitcoinDeLuxe Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the ShitcoinDeLuxe
client to ShitcoinDeLuxe Core 0.13.2 to signal for segwit support. When a
...
Since ShitcoinDeLuxe Core 0.10.0, nodes have defaulted to only relaying and
...
- `ShitcoinDeLuxe-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `ShitcoinDeLuxe-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
